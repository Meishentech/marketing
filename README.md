...
- 投標工具：可新增指定網址監測專案或主動找案專案、編輯關鍵字、設定掃描類別與保守/平衡/嚴格模式、調整專案排序，每日 08:00 自動掃描公告並保存命中網址、發布時間、關聯度分級與追蹤狀態
...
- `SUPABASE_URL`：`core/config.js` 裡的 `SB`
- `SUPABASE_SERVICE_ROLE_KEY`：Supabase Dashboard → Project Settings → API → `service_role` key（**權限很高，不可外流，只能存在這裡**）
- `RESEND_API_KEY`：若要寄 Email 通知才需要
- `TENDER_NOTIFICATION_FROM`：寄件人，例如 `美昇行銷平台 <notifications@mcttw.com.tw>`
- `TENDER_NOTIFICATION_TO`：預設收件人；也可在平台每個投標專案內填寫通知 Email

設定完成後，`.github/workflows/tender-monitor.yml` 會在每天台北時間早上 8 點自動執行 `scripts/monitor-tenders.mjs`；也可以到 GitHub repo 的 Actions 頁籤手動點 "Run workflow" 測試。
